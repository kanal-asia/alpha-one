...
import {
  ...
  resolveGoModelEnrichmentFromCatalog,
  type CanonicalModelMap,
 ...
 * available providers ΓÇö including providers the user has NOT connected yet.
...
const CANONICAL_MODEL_CACHE_TTL_MS = 5 * 60_000;
let canonicalModelCache: { at: number; value: CanonicalModelMap; found: boolean } | null = null;

/**
 * TASK-OPENCODE-085: Load the model-centric (canonical) Models.dev catalog.
 *
 * The local OpenCode snapshot is provider-centric only and carries no canonical
 * `<lab>/<model>` identity. This loads the canonical model map (the source of
 * `models/<lab>/<model>/` detail URLs) from the local OpenCode cache when a
 * model-centric file exists, otherwise fetches Models.dev's model-centric
 * `models.json` ONCE and caches it (5-min TTL). Enrichment-only and fail-open:
 * any unavailability returns an empty map, so `Model details` is simply omitted.
 */
function readCanonicalModelMap(): { canonical: CanonicalModelMap; found: boolean } {
  if (canonicalModelCache && Date.now() - canonicalModelCache.at < CANONICAL_MODEL_CACHE_TTL_MS) {
    return { canonical: canonicalModelCache.value, found: canonicalModelCache.found };
  }
  const candidates = [
    join(homedir(), ".cache", "opencode", "models-canonical.json"),
    join(homedir(), ".config", "opencode", "models-canonical.json"),
  ];
  let canonical: CanonicalModelMap = {};
  let found = false;
  for (const candidate of candidates) {
    if (!fileExists(candidate)) continue;
    try {
      const json = JSON.parse(readFileSync(candidate, "utf8")) as CanonicalModelMap;
  ...
  canonicalModelCache = { at: Date.now(), value: canonical, found };
  return { canonical, found };
}

/**
 * TASK-OPENCODE-085: Ensure the canonical model map is loaded, fetching it from
 * Models.dev's model-centric endpoint when no local canonical snapshot exists.
 * Called once per request (cached 5-min); fail-open ΓÇö on any error the map stays
 * empty and `Model details` is simply omitted.
 */
export async function ensureCanonicalModelMap(): Promise<void> {
  if (canonicalModelCache && Date.now() - canonicalModelCache.at < CANONICAL_MODEL_CACHE_TTL_MS) {
    return;
  }
  const local = readCanonicalModelMap();
  if (local.found) return;
  try {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 8000);
    const res = await fetch("https://models.dev/models.json", { signal: controller.signal });
    clearTimeout(timer);
    if (!res.ok) return;
    const json = (await res.json()) as CanonicalModelMap;
    if (json && typeof json === "object") {
      canonicalModelCache = { at: Date.now(), value: json, found: true };
    }
  } catch {
    // Fail open: canonical metadata unavailable; detail links are omitted.
  }
}

/**
 * Resolve a single runtime model (provider + slug) against the local Models.dev
 * catalog. Returns null when no usable match exists ΓÇö enrichment is optional.
 */
export function resolveModelsDevEnrichment(provider: string, slug: string): ModelsDevEnrichment | null {
  ...
  const { canonical } = readCanonicalModelMap();
  return resolveModelsDevEnrichmentFromCatalog(providers, provider, slug, canonical);
}

/**
 * TASK-085-UI: Resolve a model across ALL Models.dev providers by slug.
 * Used for OpenCode Go models which don't belong to a single known provider
 * in the Models.dev catalog.  Searches every provider for a model whose id
 * or key matches the given slug (exact or prefix match).
 */
export function resolveGoModelEnrichment(slug: string): ModelsDevEnrichment | null {
  const { providers } = readModelsDevCatalog();
  const { canonical } = readCanonicalModelMap();
  return resolveGoModelEnrichmentFromCatalog(providers, slug, canonical);
}
...
export async function fetchProviders(): Promise<ProviderSummary[]> {
  ...
  // models.dev registry and are used strictly to rank rows in the UI ΓÇö a
 ...
 * only exposes the two interactive execution modes ΓÇö it never invents modes.
 */
...
function stripBoxChars(s: string): string {
  return s.replace(/[ΓöîΓöÉΓööΓöÿΓö£ΓöñΓö¼Γö┤Γö╝ΓöÇΓöéΓò¡Γò«Γò░Γò»]/g, "");
}
